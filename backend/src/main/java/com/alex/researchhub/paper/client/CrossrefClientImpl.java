...
public class CrossrefClientImpl implements CrossrefClient {
    ...
    public CrossrefResponse fetchByDoi(String doi) {
        ...
        List<Map<String,Object>> authorList =
                (List<Map<String,Object>>) message.get("author");
        ...
        Map<String,Object> published =
                (Map<String,Object>) message.get("published-print");

        if(published == null){
            published =
                    (Map<String,Object>) message.get("published-online");
        }

        if(published != null){

            List<List<Integer>> parts =
                    (List<List<Integer>>) published.get("date-parts");
        ...
        return CrossrefResponse.builder()
                ...
                .journal(
                        ((List<String>)message.get("container-title")).isEmpty()
                                ? null
                                : ((List<String>)message.get("container-title")).get(0)
                )
                .year(year)
